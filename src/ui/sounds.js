...
export function createSoundManager({ files = DEFAULT_SOUND_FILES, masterVolume = DEFAULT_MASTER_VOLUME } = {}) {
  ...
  async function loadBufferUrl(url) {
  ...
  async function playFile(type) {
    const candidates = shuffled(soundUrls(files[type]));
    for (const url of candidates) {
      const buffer = await loadBufferUrl(url);
  ...
  function preload(types = Object.keys(files)) {
    if (muted) return;
    types.flatMap(type => soundUrls(files[type])).forEach(url => loadBufferUrl(url));
  }
